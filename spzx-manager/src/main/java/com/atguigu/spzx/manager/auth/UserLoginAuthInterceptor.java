...
public class UserLoginAuthInterceptor implements HandlerInterceptor {
    ...
    @Autowired
    RedisTemplate<String,String> redisTemplate;//注意：向redis添加数据时的redisTemplate对象，如果声明了泛型，之后从redis中取数据时，使用的redisTemplate也必须要加泛型

    //  preHandle ---》   处理器（controller方法） ---postHandle----》响应视图 --》 afterCompletion
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //OPTIONS 类型的请求，并不是一个真正的http请求，而是跨域请求的一个预检请求
        //跨域请求 = 预检请求（OPTIONS类型的一个请求）  +  真实请求

        //0、如果在拦截器中，拦截到了预检请求，直接放行即可，不需要做任何其它的处理。
        ...
        System.out.println("这里是 preHandle 方法");
    ...
    private void response208(HttpServletResponse response) {
        ...
        try {
            ...
            response.setContentType("application/json");//设置响应文档类型（如果你给客户端返回的是json数据，设置application/json）
